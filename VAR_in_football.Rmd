...
#Checar lo de la jornada 6
fechas_cl <- NULL
for (g in 1:length(fixtures_data_cl)) {
  if(fixtures_data_cl[g] == "Fixtures & Results Liga MX Clausura"){
    inicio <- g + 4
    for (h in seq(inicio,(inicio + 64),by=8)) {
      fechas_cl <- c(fechas_cl,fixtures_data_cl[inicio])
    }
  }
}

#5,13,21,29,37,45,53,61,69, ..., 81,89,97,105,113,121,129,137,145
#4,12,20,28,36,44,52,60,68


#Ejemplo de una entrada correspondiente a una jornada con todos los datos
#Como los datos interesantes los separa un "\n" entonces los separamos con base
#en ese caracter y los guardamos en forma de matriz
jornadas_cl_matrix <- str_split(jornadas_cl[1],"\n",simplify = T)
#Ejemplo de como obtener una probabilidad de la matriz de arriba
prob_ejem <- as.double(substr(str_trim(jornadas_cl_matrix[1,234]),1,2))/100

#Lo de arriba se tiene que hacer para las 18 entradas de jornadas_cl
print(prob_ejem)
