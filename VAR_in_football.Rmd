...
```{r}
#robotstxt::get_robotstxt(urltools::domain("https://projects.fivethirtyeight.com/predicciones-de-futbol/liga-mx/")) %>% cat()
web_page <- htmlParse("/Users/pedrovela/Downloads/data.html")
web_page_2 <- htmlParse("/Users/pedrovela/Documentos/doc_538.html")
results <- getNodeSet(web_page,'//div[1]') %>% sapply(., xmlValue)
results_2 <- getNodeSet(web_page_2,'//div[1]') %>% sapply(., xmlValue)
web_page_2
index <- NULL
for (i in 1:length(results)) {
  if('Monterrey' %in% results[i]){
    index <- c(index,i)
  }
}

regexpr('Cookie',results[15]) 
substr(results[15],357,359)
b <- xml_child(results)
```
