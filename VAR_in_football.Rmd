...
for (nodo in 1:length(resultados_cl)) {
  ...
  if(nodo == 145){
    ...
    marcadores_cl <- c(marcadores_cl,resultados_cl[146],"jornada 6")
  } else {
...
for (x in 1:length(resultados_cl)) {
  ...
  if(x == 145){
    ...
    prob_cl <- c(prob_cl,resultados_cl[147],"jornada 6")
  } else {
      #Sabemos que antes de los marcadores de cada jornada está la frase 
      #"Fixtures & Results Liga MX Clausura" entonces la buscamos y sabemos que 
...
for (a in 1:length(score1_cl)) {
  #En el vector de probabilidades está la opción de Jornada 6 entonces debemos quitar esa opción
  if(prob_cl[a] != 'jornada 6'){
...
**Revisar lo de que en el clausura no se trae la última probabilidad, puede ser porque esta el "jornada 6" entonces sería quitarlo y solo guardar el índice para saber en donde está**
